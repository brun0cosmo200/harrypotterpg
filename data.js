// ══════════════════════════════════════════
//  data.js — v5 | Dados estáticos completos
//  Changelog: mascotes, rebalanceamento de bônus, clã trocável
// ══════════════════════════════════════════
...
const CORES_VARINHA = [
  ...
  { id: "ebano",     hex: "#8B4513", nome: "Ébano"     },
];

// ── CLÃS ──
// Rebalanceados: bônus menores, sem combinações quebradas
const CLAS = [
  {
    id: "thorne", nome: "Thorne", raridade: "Comum", peso: 34,
    icon: "🛡️", cor: "#9aa4b2",
    bonus: { hp: 10 },
    desc: "+10 HP máximo. Clã sólido e resistente."
  },
  {
    id: "reed", nome: "Reed", raridade: "Comum", peso: 28,
    icon: "📘", cor: "#7faeea",
    bonus: { mp: 8 },
    desc: "+8 MP máximo. Ideal para quem curte soltar magia cedo."
  },
  {
    id: "blackwood", nome: "Blackwood", raridade: "Raro", peso: 16,
    icon: "🗡️", cor: "#7df0c8",
    bonus: { danoPct: 0.04 },
    desc: "+4% de dano em todas as magias."
  },
  {
    id: "rosier", nome: "Rosier", raridade: "Raro", peso: 11,
    icon: "🍀", cor: "#7de48b",
    bonus: { crit: 0.04 },
    desc: "+4% de chance crítica. Build agressiva e consistente."
  },
  {
    id: "gaunt", nome: "Gaunt", raridade: "Epico", peso: 6,
    icon: "🐍", cor: "#6fc66e",
    bonus: { dreno: 2 },
    desc: "Feitiços drenam 2 HP adicionais do alvo."
  },
  {
    id: "lestrange", nome: "Lestrange", raridade: "Epico", peso: 3.5,
    icon: "🔥", cor: "#ff8a5b",
    bonus: { danoPct: 0.06, hp: -8 },
    desc: "+6% de dano, mas -8 HP máximo. Alto risco, alto impacto."
  },
  {
    id: "malfoy", nome: "Malfoy", raridade: "Mitico", peso: 1.2,
    icon: "👑", cor: "#d6dcff",
    bonus: { desconto: 0.07, ouroPct: 0.10 },
    desc: "-7% em lojas e +10% de ouro nas batalhas."
  },
  {
    id: "potter", nome: "Potter", raridade: "Lendario", peso: 0.3,
    icon: "⚡", cor: "#ffd166",
    // Nerfado: era hp:18, mp:12, danoPct:0.08, crit:0.05
    bonus: { hp: 12, mp: 8, danoPct: 0.05, crit: 0.05 },
    desc: "Clã lendário. +12 HP, +8 MP, +5% dano e +5% crítico."
  },
];

// ── MASCOTES ──
// Sistema novo: bônus passivos, 1 por vez, obtidos via giros
const MASCOTES = [
  {
    id: "coruja", nome: "Coruja", emoji: "🦉", raridade: "Comum", peso: 35,
    desc: "+8% de XP em todas as batalhas",
    bonus: { xpPct: 0.08 }
  },
  {
    id: "sapo", nome: "Sapo", emoji: "🐸", raridade: "Comum", peso: 30,
    desc: "+12 HP máximo permanente",
    bonus: { hp: 12 }
  },
  {
    id: "gato", nome: "Gato", emoji: "🐱", raridade: "Raro", peso: 18,
    desc: "+5% de chance de crítico",
    bonus: { crit: 0.05 }
  },
  {
    id: "rato", nome: "Rato", emoji: "🐀", raridade: "Raro", peso: 12,
    desc: "+15% de ouro em todas as batalhas",
    bonus: { ouroPct: 0.15 }
  },
  {
    id: "niffler", nome: "Niffler", emoji: "🦡", raridade: "Epico", peso: 4,
    desc: "+25% de ouro e +5% de XP",
    bonus: { ouroPct: 0.25, xpPct: 0.05 }
  },
  {
    id: "fenix", nome: "Fênix", emoji: "🕊️", raridade: "Lendario", peso: 1,
    desc: "Ressurge com 25 HP uma vez por masmorra",
    bonus: { ressurgirMascote: 25 }
  },
];

function sortearMascote() {
  const total = MASCOTES.reduce((a, m) => a + m.peso, 0);
  let roll = Math.random() * total;
  for (const m of MASCOTES) { roll -= m.peso; if (roll <= 0) return m; }
  return MASCOTES[0];
}
...
const ATAQUE_BASICO = {
  id: "__basico__", nome: "Ataque Básico", icon: "🪄",
  dano: [6,12], mp: 0, desc: "Golpe sem magia"
};

// ── ÁRVORE DE HABILIDADES ──
// Rebalanceada: bônus menores, cap de dano respeitado
const ARVORE_HABILIDADES = {
  // Tier 1 — Nível 5
  5: [
    { id: "foco",        nome: "Foco de Batalha",   icon: "🎯", desc: "+8% de dano em todos os feitiços",          tipo: "dano_pct",  valor: 0.08 },
    { id: "resistencia", nome: "Resistência Mágica", icon: "🛡️", desc: "+20 HP máximo permanente",                  tipo: "hp_max",    valor: 20   },
  ...
  10: [
    ...
    { id: "vampirismo",    nome: "Vampirismo",     icon: "🩸", desc: "+6% do dano causado é convertido em HP",             tipo: "vampirismo",    valor: 0.06 },
  ],
  // Tier 3 — Nível 15
  15: [
    { id: "frenesi",    nome: "Frenesi",       icon: "🔥", desc: "Cada kill aumenta dano em 3% (acumula, reseta ao morrer)", tipo: "frenesi",    valor: 0.03 },
    { id: "mana_surge", nome: "Surge de Mana", icon: "💧", desc: "Feitiços com MP > 30 causam +18% de dano",               tipo: "mana_surge", valor: 0.18 },
    { id: "fortress",   nome: "Fortaleza",     icon: "🏰", desc: "-15% de dano recebido de todos os inimigos",             tipo: "dmg_reduc",  valor: 0.15 },
  ],
  // Tier 4 — Nível 20
  20: [
  ...
  // Tier 5 — Nível 25
  25: [
    { id: "devastacao",  nome: "Devastação",  icon: "💥", desc: "+20% de dano mas -12 HP máximo",            tipo: "devastacao", valor: 0.20 },
    { id: "arcano_puro", nome: "Arcano Puro", icon: "🔮", desc: "Feitiços da casa custam -25% de MP",         tipo: "mp_reduc",   valor: 0.25 },
    { id: "lendario",    nome: "Lendário",    icon: "⚜️", desc: "+30 HP máx, +15 MP máx, +5% dano — o equilíbrio perfeito", tipo: "lendario", valor: 1 },
  ],
  // Tier 6 — Nível 30
  30: [
...
const DIARIO_EVENTOS = {
  ...
  mascote:         "Ganhei um companheiro hoje. Há algo reconfortante em não caminhar sozinho por esses corredores.",
...
const LOJA_ITENS = [
  ...
  // Giros de clã
  { id: "giro_cla",      nome: "Giro de Família",        icon: "🎰", preco: 1800, desc: "Ganha 1 giro extra na roleta de clã", tipo: "cla",        nivelMin:  2 },
  // Giros de mascote — NOVO
  { id: "giro_mascote",  nome: "Giro de Mascote",        icon: "🎠", preco: 1200, desc: "Ganha 1 giro na roleta de mascotes",  tipo: "mascote",    nivelMin:  2 },
  // Poções de XP
  ...
  // Permanentes (limite de 1 compra cada)
...
const CONQUISTAS_DEF = [
  ...
  { id: "mascote_lendario", nome: "Companheiro Lendário",  desc: "Obtenha a Fênix como mascote",                icon: "🕊️",  condicao: (s) => s.mascoteId === 'fenix' },
